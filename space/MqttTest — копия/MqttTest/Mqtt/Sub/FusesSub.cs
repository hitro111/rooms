        ...
        //: base("localhost")
        {
            ...
            On["space/fuses/out"] = _ =>
            {
                if (_.Message == FusesOk)
                    _mainForm.Invoke((MethodInvoker) (() => _mainForm.SetFuses(true, false)));

                else if (_.Message == FusesFail)
                    _mainForm.Invoke((MethodInvoker)(() => _mainForm.SetFuses(false, false)));

                else if (_.Message == FusesOkSilent)
                    _mainForm.Invoke((MethodInvoker)(() => _mainForm.SetFuses(true, true)));

                else if (_.Message == FusesFailSilent)
                    _mainForm.Invoke((MethodInvoker)(() => _mainForm.SetFuses(false, true)));
            };
        }
    }
}
