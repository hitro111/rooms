...
namespace MqttTest.Mqtt.Sub
{
    public class ManipSub : MqttModule
    {
        ...
        //: base("localhost")
        {
            ...
            {
                if (_.Message == Ok)
                    _mainForm.Invoke((MethodInvoker) (() => _mainForm.SetManip(true, false)));

                else if (_.Message == OkSilent)
                    _mainForm.Invoke((MethodInvoker)(() => _mainForm.SetManip(true, true)));

                else if (_.Message == NoSilent)
                    _mainForm.Invoke((MethodInvoker)(() => _mainForm.SetManip(false, true)));
            };
        }
    }
}
