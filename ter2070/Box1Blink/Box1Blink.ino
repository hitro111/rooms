...
#define ACC "tboxblink"
byte mac[] = { 0x03, 0xAD, 0xBE, 0x01, 0xAD, 0xBE };
byte ip[] = { 192, 168, 0, 53 }; // <- change to match your network
...
void loop() {
  ...
  if (customKey) {
    ...
    if (curPos == PWD_SIZE)
    {
      if (pwd[0] == '2' && pwd[1] == '7' && pwd[2] == '8' && pwd[3] == '4')
      {
        setOpenText();
        isOk = true;
...
void handleBlink()
{
  ...
  else
  {
    if (millis() - lastEvent > offDelay)
    {
      ...
      curBlinked++;
      if (curBlinked >= blinks[curDig])
      {
        curBlinked = 0;
        curDig = (curDig + 1) % PWD_SIZE;
      }
...
void connect() {
  ...
  client.subscribe("ter2070/tboxblink/reset");
